...
def _seed_worker(worker_id):
    """Seed tiap DataLoader worker deterministik (dari initial_seed generator).
    Bikin augmentasi + urutan data IDENTIK lintas arsitektur di seed sama,
    dan INVARIAN terhadap num_workers (menutup saga num_workers)."""
...
def train_one_run(cfg, dataset_name, model_key, seed, epochs=None, logger=None,
                  recipe="adamw", arm=None, grayscale=False):
    ...
    grayscale=True (arm R2.3): latih pada input grayscale (train + val loader)."""
    t0 = time.time()
    tcfg = cfg["train"]
    ...
    val_ds = data_mod.get_dataset(dataset_name, "val", cfg, train_aug=False,
                                  grayscale=grayscale)
    # generator ter-seed -> urutan shuffle deterministik & INDEPENDEN dari init model
    # (arsitektur beda, seed sama -> urutan data sama). worker_init_fn -> augmentasi
    # deterministik & invarian num_workers.
    g = torch.Generator()
    g.manual_seed(seed)
    ...
    for attempt, cur_lr in enumerate(lr_attempts):
        ...
        # kontrak spec: print baris loading pretrained tiap run (verifikasi tag)
        log(f"[{arm}][seed{seed}] loading {cfg['models'][model_key]['timm_name']} "
            f"| train_mode={'gray' if grayscale else 'rgb'}")
        ...
        log(f"[train] {dataset_name}/{arm}/seed{seed} recipe={recipe} "
            ...
            f"grayscale={grayscale} epochs={epochs} device={device} "
            f"n_train={len(train_ds)} n_val={len(val_ds)} bs={bs} accum={accum}"
        ...
        for epoch in range(epochs):
            ...
            if val_m["acc"] > best_acc:
                ...
                torch.save({"model_key": model_key, "arm": arm, "recipe": recipe,
                            ...
                            "seed": seed, "epoch": epoch + 1, "grayscale": grayscale,
